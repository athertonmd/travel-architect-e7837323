...
import { StandardFonts, rgb } from "https://esm.sh/pdf-lib@1.17.1";
import { PdfSettings } from "../types/pdfTypes.ts";
import { drawText, drawDivider } from "./textUtils.ts";

// Simplified header drawing with embedded image
export const addHeader = async (pdfDoc: any, page: any, font: any, yPosition: number, settings?: PdfSettings) => {
  ...
  try {
    ...
    if (settings?.logoUrl) {
      try {
        // If user provided a custom logo URL, try to fetch it
        console.log("Fetching custom logo from URL:", settings.logoUrl);
        ...
        logoBytes = new Uint8Array(atob(base64Data).split('').map(char => char.charCodeAt(0)));
      }
    } else {
    ...
    // If there's a company name or header text, add it
    if (settings?.companyName) {
      const yText = yPosition - scaledHeight - 25;
      const headerFont = await pdfDoc.embedFont(StandardFonts.HelveticaBold);
      drawText(page, settings.companyName, width / 2 - 100, yText, headerFont, 16, primaryColor);
    }
    
    if (settings?.headerText) {
      const yText = yPosition - scaledHeight - (settings.companyName ? 50 : 25);
      drawText(page, settings.headerText, width / 2 - 100, yText, font, 12, primaryColor);
    }

    // Add a decorative line under the logo
    const lineY = yPosition - scaledHeight - (settings?.headerText || settings?.companyName ? 60 : 20);
    drawDivider(page, lineY, 495, rgb(0.8, 0.8, 0.8));
